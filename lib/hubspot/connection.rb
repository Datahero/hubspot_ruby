module Hubspot
  class Connection
    ...
    class << self
      def get_json(path, opts, props="")
        url = generate_url(path, opts)
        if props
          url = url + props
        end
        puts url
        auth = "Bearer " + Hubspot::Config.hapikey
        response = get(url, headers: { 'Content-Type' => 'application/json', 'Authorization' => auth }, format: :json)
      ...
      def post_json(path, opts)
        ...
        response = post(url, body: opts[:body].to_json, headers: { 'Content-Type' => 'application/json', 'Authorization' => auth }, format: :json)
      ...
      def put_json(path, opts)
        url = generate_url(path, opts[:params])
        auth = "Bearer " + Hubspot::Config.hapikey
        response = put(url, body: opts[:body].to_json, headers: { 'Content-Type' => 'application/json', 'Authorization' => auth }, format: :json)
        log_request_and_response url, response, opts[:body]
        raise(Hubspot::RequestError.new(response)) unless response.success?
        response.parsed_response
      end
      ...
      def generate_url(path, params={}, options={})
        ...
        #params["access_token"] = Hubspot::Config.hapikey unless options[:hapikey] == false
  ...
  class FormsConnection < Connection
    ...
    def self.submit(path, opts)
      ...
      post(url, body: opts[:body], headers: { 'Content-Type' => 'application/x-www-form-urlencoded', 'Authorization' => auth})
    end
  end
end
